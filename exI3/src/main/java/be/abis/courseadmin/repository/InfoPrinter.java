...
public class InfoPrinter implements Printer {
    ...
    public void printSalaryHistory(Instructor i) throws IOException, AgeException {
        int seniority =0;
        double salary=i.getStartSalary();
        int sAge=i.getStartAge();
        BufferedWriter bw = Files.newBufferedWriter(Paths.get("exI3\\src\\salaryhistory"+i.getFirstName()+".txt"));
    ...
    public void printInfo(Course c,String fileName) throws IOException {
        BufferedWriter bw = Files.newBufferedWriter(Paths.get("exI3\\src\\"+fileName));
