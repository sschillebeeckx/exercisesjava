...
public class InfoPrinter implements Printer {
    ...
    public void printSalaryHistory(Instructor i) throws IOException, PersonShouldBeAdultException {
        int seniority =0;
        double salary=i.getStartSalary();
        int sAge=i.getStartAge();
        BufferedWriter bw = Files.newBufferedWriter(Paths.get("exH1\\src\\salaryhistory"+i.getFirstName()+".txt"));
    ...
    public void printInfo(Course c,String fileName) throws IOException {
        BufferedWriter bw = Files.newBufferedWriter(Paths.get("exH1\\src\\"+fileName));
